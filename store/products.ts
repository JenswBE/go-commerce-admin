    ...
    return this.$api.products
      ...
      .catch((e) => {
        const alert: Alert = {
          ...
          message:
            this.$i18n
              .t('fetchItemFailed', { item: this.$i18n.tc('product', 2) })
              .toString() + `: ${e.message}`,
        }
    ...
    return this.$api.products
      ...
      .catch((e) => {
        const alert: Alert = {
          ...
          message:
            this.$i18n
              .t('addItemFailed', { item: this.$i18n.tc('product', 1) })
              .toString() + `: ${e.message}`,
        }
        context.commit('general/SET_ALERT', alert, { root: true })
      })
    ...
    return this.$api.products
      ...
      .catch((e) => {
        const alert: Alert = {
          ...
          message:
            this.$i18n
              .t('updateItemFailed', { item: this.$i18n.tc('product', 1) })
              .toString() + `: ${e.message}`,
        }
        context.commit('general/SET_ALERT', alert, { root: true })
      })
    ...
    return this.$api.products
      ...
      .catch((e) => {
        const alert: Alert = {
          ...
          message:
            this.$i18n
              .t('deleteItemFailed', { item: this.$i18n.tc('product', 1) })
              .toString() + `: ${e.message}`,
        }
        context.commit('general/SET_ALERT', alert, { root: true })
      })
